        ...
        <div className="flex flex-col items-center justify-items-center gap-2">
          <h1 className="text-2xl font-medium">form</h1>
              <form action={formAction}>
                ...
                </Select><label className="text-sm/6 font-medium text-red-900">{producttypeErr}</label><br/>
                {isShown && <Fragment>
                <Label>载重(千克)　　　</Label>
                <Select name="weight" value={String(width)} onValueChange={(e) => setWeight(Number(e))}>
                  <SelectTrigger>
                    <SelectValue placeholder=""/>
                  </SelectTrigger>
                  <SelectContent>
                ...
                <Input type="text" name="weight1" value={width1} onChange={(e) => setWeight1(e.target.value)} /><label className="text-sm/6 font-medium text-red-900">{weightErr}</label><br/>
                <Label>轿厢宽度(毫米)</Label>
                <Select name="width" value={String(width)} onValueChange={(e) => setWidth(Number(e))}>
                  <SelectTrigger>
                    <SelectValue placeholder=""/>
                  </SelectTrigger>
                  <SelectContent>
                ...
                <Input type="text" name="width1" value={width1} onChange={(e) => setWidth1(e.target.value)} /><label className="text-sm/6 font-medium text-red-900">{widthErr}</label><br/>
                <Label>轿厢深度(毫米)</Label>
                <Select name="height" value={String(height)} onValueChange={(e) => setHeight(Number(e))}>
                  <SelectTrigger>
                    <SelectValue placeholder=""/>
                  </SelectTrigger>
                  <SelectContent>
                ...
                <Input type="text" name="height1" value={height1} onChange={(e) => setHeight1(e.target.value)} /><label className="text-sm/6 font-medium text-red-900">{heightErr}</label><br/>
                </Fragment>}

...
int main(int argc, char** argv) {
    ...
    for (;;) {
        ...
        //
        img = cv::Scalar::all(0);
        cv::circle(img, phi2xy(z_k), 4, cv::Scalar(128, 255, 255));  // observed (in yellow)
        cv::circle(img, phi2xy(y_k), 4, cv::Scalar(255, 255, 255), 2);  // predicted (in white)
        cv::circle(img, phi2xy(x_k), 4, cv::Scalar(0, 0, 255));  // actual to (in red)

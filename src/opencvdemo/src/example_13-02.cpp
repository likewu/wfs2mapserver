...
int main( int argc, char** argv ) {
  string argv11[]={"","HandIndoorColor.jpg","HandOutdoorColor.jpg","HandOutdoorSunColor.jpg","fruits.jpg"};
  //if( argc != 5 ) { help( argv ); return -1; }
  vector<cv::Mat> src(5);
  cv::Mat tmp;
  int i;

  tmp = cv::imread( "E:/app/julia/Learning-OpenCV-3_examples/"+argv11[1], 1);
  if( tmp.empty() ) {
  ...
  //
  for(i = 2; i<5; ++i){
    src[i] = cv::imread("E:/app/julia/Learning-OpenCV-3_examples/"+argv11[i], 1);
    if(src[i].empty()) {
